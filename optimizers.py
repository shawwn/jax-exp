...
Callable[[State], Params]]]) -> Callable[..., Optimizer]:
  ...
  def tree_opt_maker(*args, **kwargs):
    ...
    def tree_update(i, grad_tree, opt_state, *args, **kws):
      ...
      new_states = [update(i, g, state, *args, **kws) if g is not None and state is not None else state for g, state in zip(grad_flat, states)]
...
def sgd(step_size):
  ...
  def init(x0):
    return x0
  def update(i, g, x, settings=None):
    if settings:
      nonlocal step_size
      if 'step_size' in settings: step_size = make_schedule(settings['step_size'])
...
def adam_std(step_size, b1=0.9, b2=0.999, eps=1e-8):
  ...
  def init(x0):
    ...
    v0 = jnp.zeros_like(x0)
    return x0, m0, v0
  def update(i, g, state, settings=None):
    if settings:
      nonlocal step_size, b1, b2, eps
      if 'step_size' in settings: step_size = make_schedule(settings['step_size'])
      if 'b1' in settings: b1 = settings['b1']
      if 'b2' in settings: b2 = settings['b2']
      if 'eps' in settings: eps = settings['eps']
    x, m, v = state
...
@optimizer
def wednesday(step_size, *, b1=0.9, b2=0.999, b3=0.0, b4=0.0, weight_decay=0.0):
  """Construct optimizer triple for Wednesday.
  Args:
    step_size: positive scalar, or a callable representing a step size schedule
      that maps the iteration index to a positive scalar.
    b1: optional, a positive scalar value for beta_1, the exponential decay rate
  ...
  def update(i, g, state, settings=None):
    if settings:
      nonlocal step_size, b1, b2, b3, b4, weight_decay
      if 'step_size' in settings: step_size = make_schedule(settings['step_size'])
      if 'b1' in settings: b1 = settings['b1']
      if 'b2' in settings: b2 = settings['b2']
      if 'b3' in settings: b3 = settings['b3']
      if 'b4' in settings: b4 = settings['b4']
      if 'weight_decay' in settings: weight_decay = settings['weight_decay']

    x, m = state
    lr = step_size(i)

    # Apply weight decay.
    x = x * (1.0 - lr * weight_decay * (len(x) > 1))
    ...
    # speed = jnp.abs(g)
    # speed = lerp(jnp.abs(g), jnp.abs(velocity), b2)
    # speed = jnp.abs(velocity)
    speed =  lr * lerp(jnp.abs(g), jnp.abs(velocity), b3)
    ...
    # Additive weight decay.
    x_new = x_new + x * b4 * (len(x) > 1)
...
@optimizer
def lion(step_size, b1=0.9, b2=0.999, weight_decay=0.0):
  """Construct optimizer triple for Adam.
  Args:
    step_size: positive scalar, or a callable representing a step size schedule
      that maps the iteration index to a positive scalar.
  ...
  def update(i, g, state, settings=None):
    ...
    if settings:
      nonlocal step_size, b1, b2, weight_decay
      if 'weight_decay' in settings: weight_decay = settings['weight_decay']
      if 'step_size' in settings: step_size = make_schedule(settings['step_size'])
      if 'b1' in settings: b1 = settings['b1']
      if 'b2' in settings: b2 = settings['b2']
