...
def adam(step_size, b1=0.9, b2=0.999, eps=1e-8):
  ...
  def update(i, g, state):
    ...
    # mhat = p / (1 - jnp.asarray(b1, p.dtype) ** (i + 1))  # Bias correction.
    # vhat = v / (1 - jnp.asarray(b2, p.dtype) ** (i + 1))
    # x = x - step_size(i) * phat / (jnp.sqrt(vhat) + eps)
    def lerp(x, y, t):
      return x + t * (y - x)
      # return (1 - t) * x + t * y
    # p, v = jnp.where(i > 0, (p, v), (g, g_2))
    # G = jnp.sqrt(jnp.square(g))
    if False:
      ok = i > 0
      p = jnp.where(ok, p, g)
      v = jnp.where(ok, v, G)
      ...
      def ready(x, g):
        return x
      branches = [accum if idx < n else ready for idx in range(n + 1)]
      p = jax.lax.switch(i, branches, p, g)
      v = jax.lax.switch(i, branches, v, G)
    else:
      n_accum = 10
      def accum(i, g, state):
        x, p, v = state
      ...
      def apply(i, g, state):
        x, p, v = state
        G = jnp.sqrt(jnp.square(g))
        p = lerp(g, p, b1) # First moment estimate.
        v = lerp(G, v, b2) # Second moment estimate.
        #x = x - step_size(i) * p / v
        zero_to_one = (1 - (p / v)) / 2
        # x = x - step_size(i) * g * zero_to_one
        # t = jnp.square(zero_to_one)
        t = zero_to_one
        x = lerp(x, -g * t, step_size(i))
        return x, p, v
      index = i - n_accum
      branches = [accum, apply]
      # branches = [accum, accum]
      return jax.lax.switch(index, branches, i, g, state)
    # p = lerp(g, p, b1) # First moment estimate.
    # v = lerp(G, v, b2) # Second moment estimate.
    # x = x - step_size(i) * p / v
    # # def bias(b, i):
    # #   return 1 - jnp.asarray(b, p.dtype) ** (i + 1)
    # # phat = p / bias(b1, i)  # Bias correction.
    # # vhat = v / bias(b2, i)
    # return x, p, v
  def get_params(state):
    x, _, _ = state
